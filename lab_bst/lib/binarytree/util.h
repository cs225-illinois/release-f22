...
double pemdasToAST(const std::string& pemdas_str, typename BST<std::string>::Node*& t, std::vector<std::pair<typename BST<std::string>::Node*, double>>& subtrees)
{
    ...
    for (size_t i = 0; i < spaceless.length(); ++i)
    {
        if (spaceless[i] == '(')
        {
            ...
            typename BST<std::string>::Node* temp_tree;
    ...
    {
        if (tokenize[i] == '+') {
            t = new typename BST<std::string>::Node("+");
    ...
    t = new typename BST<std::string>::Node(tokenize);
    return std::stod(tokenize);
}

double pemdasToAST(const std::string& pemdas_str, typename BST<std::string>::Node*& t) {
    std::vector<std::pair<typename BST<std::string>::Node*, double>> subtrees;
